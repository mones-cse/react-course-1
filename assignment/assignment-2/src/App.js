...
class App extends Component {
    ...
    deleteChar = (index) => {
        let newString = this.state.inputText.slice(0, index) + this.state.inputText.slice(index + 1)
        this.setState({inputText: newString})
    }
    ...
    render() {
        return (
            <div className="App">
                ...
                <p>The Text Length {this.state.inputText.length}</p>
                <ValidationComponent inputTextLength={this.state.inputText.length}/>
                <CharComponent currentText={this.state.inputText} click={this.deleteChar}/>
